...
public final class AppleAPI: Sendable {
    public let session = AppleTransport()
        ...
        guard let array = response["teams"] as? [[String: Any]] else {
            if let result = response["resultCode"] {
                ...
                if let data = response["error"] as? String {
                    throw AppleAPIError.customError(code: 0, message: "Apple returned an API error; response body withheld.")
                }
    ...
    public func updateAppID(_ appID: AppID, capabilities: [String], team: Team, session: AppleAPISession) async throws -> AppID {
        return try await updateCapabilitiesVerified(appID, capabilities: capabilities, team: team, session: session)
    }
    ...
    public func fetchProvisioningProfileForAppID(appID: AppID, deviceType: DeviceType, team: Team, session: AppleAPISession) async throws -> ProvisioningProfile {
        ...
        guard let encoded = dictionary["encodedProfile"] as? Data else { throw AppleAPIError.badServerResponse }
        var decoded = try ProfileValidation.decodeCMS(encoded)
        decoded["encodedProfile"] = encoded
        decoded["provisioningProfileId"] = dictionary["provisioningProfileId"]
        let provisioningProfile = ProvisioningProfile(decoded)
